...
@Component({
  selector: 'app-myorders-cancelled',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './myorders-cancelled.html',
  styleUrl: './myorders-cancelled.css',
})
export class MyordersCancelled implements OnInit {
  ...
  cargarPedidos() {
    ...
    this.pedidosService.getByCliente(codCliente).subscribe({
      next: (data) => {
        // Filtramos solo los cancelados (estado 3)
        const cancelados = data.filter((p) => p.estado === 3);
        this.pedidosRaw.set(cancelados);
        this.loadingList.set(false);
      },
      error: (err) => {
        console.error(err);
        this.loadingList.set(false);
        this.toastService.error('Error al cargar mis pedidos cancelados');
      },
    });
  }
